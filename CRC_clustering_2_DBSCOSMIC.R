...
filepath <- "/home/shivalik2/Soumik/my_codes/Avik_bhaiya_project/dbs2_COSMIC_variant//"

#======================================================================
# Log Normalisation DBS2-COSMIC Signatures
#======================================================================
# Log normalization + z-score (row-wise)
log_data <- log1p(df_dbs2)  # log(1 + x) to handle zeros
df_scaled <- t(scale(t(log_data)))  # z-score per patient (row-wise)
df_scaled <- as.data.frame(df_scaled)

# Save the scaled dataset
write.csv(t(df_scaled), paste0(filepath, "df_scaled_dbs2_zscore.csv"), row.names = T)
...
dbs2_cluster <- kmean_clusters_df
dbs2_cluster$PATIENT_IDs <- rownames(dbs2_cluster)
colnames(dbs2_cluster)[1] <- "Cluster"
rownames(dbs2_cluster) <- 1:nrow(dbs2_cluster)
dbs2_cluster <- dbs2_cluster[, c(2, 1)]  # Reorder columns for clarity

# Save the clustering results
write_csv(dbs2_cluster, paste0(filepath, "kmean_clusters_sbs2.csv"))
...
df_htmp <- as.data.frame(fread(paste0(filepath, "df_scaled_dbs2_zscore.csv"), sep = ",", header = TRUE))
