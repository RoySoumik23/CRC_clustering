...
km_df$`DNA Tumor Sample Barcode` <- rownames(km_df)
...
colnames(km_df)[colnames(km_df) == "Overall survival days"] <- "OS_MONTHS"
colnames(km_df)[colnames(km_df) == "Vital Status"] <- "OS_STATUS"
colnames(km_df)[colnames(km_df) == "DNA Tumor Sample Barcode"] <- "PATIENT_IDs"
...
km_df$Sex <- factor(km_df$Sex)
km_df$`Tumour Stage` <- factor(km_df$`Tumour Stage`)
km_df$`Tumour Grade` <- factor(km_df$`Tumour Grade`)
km_df$`MSI Status` <- factor(km_df$`MSI Status`)
km_df$`CMS Tumour` <- factor(km_df$`CMS Tumour`)
...
for (pair in cluster_pairs) {
  ...
  pair_df <- subset(km_df, class %in% c(paste0("CRC_cluster ", cluster_a), paste0("CRC_cluster ", cluster_b)))
  
  # Drop unused factor levels
  pair_df$class <- droplevels(pair_df$class)
  
  # Fit KM curve
  kmcurve_pair <- survfit(Surv(OS_MONTHS, OS_STATUS) ~ class, data = pair_df)
  
  # Fit Cox PH model for this pair
  coxph_model_pair <- coxph(Surv(OS_MONTHS, OS_STATUS) ~ class + `Age at diagnosis` + Sex + 
  ...
  file_name <- paste0("KM_OS_cluster_", cluster_a, "_vs_", cluster_b, ".tif")
